...
import { RiCheckLine, RiCodeSSlashLine, RiEyeLine, RiFileCopyLine, RiRestartLine } from "@remixicon/react";
  ...
  return <article className="block-example">
    <div className="showcase-toolbar">
      <div className="preview-tabs" role="tablist" aria-label={`${block.title} view`}>
        {[{ key: "preview", label: "Preview", icon: RiEyeLine }, { key: "code", label: "Source", icon: RiCodeSSlashLine }].map((t) => <button key={t.key} id={`${id}-${t.key}`} role="tab" aria-selected={view === t.key} aria-controls={`${id}-panel`} onClick={() => setView(t.key)}><t.icon size={14} />{t.label}</button>)}
      </div>
      <div className="showcase-tools"><button className="studio-icon-button" onClick={() => setRevision((r) => r + 1)} aria-label={`Reset ${block.title}`} title="Reset example"><RiRestartLine size={14} /></button><button className="block-copy-button" onClick={() => copy(source)}>{copied ? <RiCheckLine size={13} /> : <RiFileCopyLine size={13} />}{copied ? "Copied" : "Copy source"}</button></div>
    </div>
