...
import { RiAlignCenter, RiAlignLeft, RiAlignRight, RiBankCardLine, RiBold, RiGlobalLine, RiInboxLine, RiItalic, RiLink, RiRocketLine, RiSettings3Line, RiTeamLine, RiUser3Line } from "@remixicon/react";
...
export function ComponentsIndex({ navigate }: { navigate: (t: string) => void }) {
  ...
  return (
    <>
      <PageHeader title="Component library" description="The pieces of a cohesive interface. Explore examples, inspect the API, and try the interactions in each component's documentation." tags={[`${total} documentation pages`, "Free during beta"]} />
      <div className="component-index-filter"><Input aria-label="Filter components" placeholder="Find a component..." startContent={<Search />} value={q} onChange={(e) => setQ(e.target.value)} /><span role="status">{groups.reduce((n, g) => n + g.items.length, 0)} results</span></div>
      {groups.map((g) => (
          ...
          <div className="component-index-grid">
            {g.items.map((it) => (
              <article key={it.href} className="component-index-item">
                ...
                <a href={`#/${it.href}`} className="component-index-link" onClick={(e) => { e.preventDefault(); navigate(it.href); }}><span>{it.title}</span><ArrowRight size={15} /></a>
              </article>
            ))}
          </div>
        </section>
      ))}
      {!groups.length && <div className="search-empty"><Search size={24} /><h2>No components found</h2><p>Try another component name or clear the filter.</p><button className="text-action" onClick={() => setQ("")}>Clear search</button></div>}
    </>
  );
}
