  ...
  return <main id="main" tabIndex={-1} className="beta-page">
    <div className="home-container">
      ...
      <div className="beta-access-list">
        {[{ icon: RiCodeSSlashLine, title: `${count} component documentation pages`, description: "Examples, API references, and an editable React playground.", href: "components" }, { icon: RiStackLine, title: `${BLOCKS.length} composed block examples`, description: "Actual-size previews and copyable implementation source.", href: "blocks" }, { icon: RiPaletteLine, title: "Theme Studio and foundations", description: "Customize the accent and radius. Export the variables you use.", href: "theme" }].map((item) => <a href={`#/${item.href}`} key={item.href}><item.icon size={20} /><div><h2>{item.title}</h2><p>{item.description}</p></div><RiCheckLine size={17} /></a>)}
      </div>
