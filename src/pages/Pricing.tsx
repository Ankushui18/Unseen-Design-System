import { ArrowRight, Check, Code2, Layers, Palette } from "lucide-react";
...
export function PricingPage({ navigate }: { navigate: (to: string) => void }) {
  ...
  return <main id="main" tabIndex={-1} className="beta-page">
    <div className="home-container">
      <header className="beta-page-heading page-enter"><span className="beta-page-label"><span className="beta-status-dot" /> Public beta</span><h1>Everything here.<br />Free to explore.</h1><p>We are refining Aperture in public. Try the components, test the interactions, and help us get the details right before a paid launch.</p><Button size="lg" tone="default" onClick={() => navigate("components")} endContent={<ArrowRight />}>Explore the library</Button></header>
      <div className="beta-access-list">
        {[{ icon: Code2, title: `${count} component documentation pages`, description: "Examples, API references, and an editable React playground.", href: "components" }, { icon: Layers, title: `${BLOCKS.length} composed block examples`, description: "Actual-size previews and copyable implementation source.", href: "blocks" }, { icon: Palette, title: "Theme Studio and foundations", description: "Customize the accent and radius. Export the variables you use.", href: "theme" }].map((item) => <a href={`#/${item.href}`} key={item.href}><item.icon size={20} /><div><h2>{item.title}</h2><p>{item.description}</p></div><Check size={17} /></a>)}
      </div>
