...
export function InstallationPage({ navigate }: { navigate: (to: string) => void }) {
  ...
  return <>
    ...
    <Section title="1. Get the source" description="Download the current source as a JSON file map, or copy an individual block from its Source tab. Preserve the directory structure so relative imports resolve.">
      <div className="installation-download"><div><h3>Source available in this build</h3><p>Components, theme provider, hooks, utilities, and CSS.</p></div><Button variant="outline" tone="default" startContent={<Download />} onClick={download}>Download source map</Button></div>
    ...
    <Section title="2. Install dependencies" description="Start with a React + TypeScript project and configure Tailwind CSS v4 for your build tool. The shared components use these dependencies:">
      <Snippet>npm install lucide-react @remixicon/react clsx tailwind-merge</Snippet>
...
// Result: ${cn("text-label-sm", "text-foreground")}`} />
      <div className="flex flex-wrap gap-3"><Button onClick={() => navigate("components/button")} endContent={<ArrowRight />}>Try a component</Button><Button variant="outline" tone="default" onClick={() => navigate("theme")}>Customize the theme</Button></div>
    </Section>
  </>;
}