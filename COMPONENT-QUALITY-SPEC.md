...
| **Type** | The four families only: `title-h1…h6`, `label-xl…xs`, `paragraph-xl…xs`, `subheading-*`. Weights are **400 / 500** — nothing else. Labels use `label-*` (500); body copy uses `paragraph-*` (400). | `text-sm`/`text-xl`, arbitrary `text-[13px]`, `font-semibold`, mixed weights inside one control. |
...
- **Design-lint:** zero findings for the component's file on: raw hex, raw palette colour, arbitrary type sizes, off-scale weights, legacy elevation, card-border pattern, undersized icons.
