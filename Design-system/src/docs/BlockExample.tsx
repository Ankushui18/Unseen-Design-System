...
export function BlockExample({ block }: { block: BlockDef }) {
  ...
  return <article className="block-example">
    <header className="block-example-heading"><div><h2>{block.title}</h2><p>{block.description}</p></div><span className="block-free-label">Free in beta</span></header>
    ...
    <footer className="block-example-footer"><span>{block.category}</span><span>Interactive example <span aria-hidden="true">/</span> Native size, no scaling</span></footer>
  </article>;
}